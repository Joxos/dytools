...
```bash
dytools cluster -r 6657 --threshold 0.5 --limit 50
```
...
```bash
dytools import data.csv -r 6657
```

#### export
将数据库数据导出为 CSV 文件。
```bash
dytools export -r 6657 -o backup.csv
```
...
- [ ] 历史数据迁移 — 运行 migrate_room_id.zsh 将旧数据统一为新的 room_id 格式
